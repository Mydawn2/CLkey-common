...
import (
	...
	bcx509 "github.com/Mydawn2/CLkey-common/crypto/x509"
	"github.com/tjfoc/gmsm/sm2"

	"github.com/Mydawn2/CLkey-common/crypto"
	"github.com/Mydawn2/CLkey-common/crypto/asym"
	"github.com/Mydawn2/CLkey-common/crypto/hash"
)
...
func CreatePrivKey(keyType crypto.KeyType, privKeyPath, privKeyFileName string, uid []byte) (crypto.PrivateKey, error) {
	algoName, ok := crypto.KeyType2NameMap[keyType]
	if !ok {
		return nil, fmt.Errorf("unknown key algo type [%d]", keyType)
	}

	privKey, err := asym.GenerateKeyPair(keyType, uid)
	if err != nil {
...
func CreateCSR(cfg *CSRConfig) error {
	templateX509, err := GenerateCSRTemplate(cfg.PrivKey, cfg.Country, cfg.Locality, cfg.Province, cfg.OrganizationalUnit, cfg.Organization, cfg.CommonName)
	if err != nil {
