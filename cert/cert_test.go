...
func TestCreatePrivKeyWithDiffKeyTypes(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			_, err := CreatePrivKey(crypto.KeyType(tt.keyType), "", "", []byte("mydawn"))
...
func TestCreatePrivKey(t *testing.T) {
	...
	key, err := CreatePrivKey(crypto.ECC_NISTP256, testFilePath, testFileName, []byte("mydawn"))
...
func TestCreateCACertificate(t *testing.T) {
	...
	key, err := CreatePrivKey(crypto.ECC_NISTP256, "", "", []byte("mydawn"))
	cfg = &CACertificateConfig{
		PrivKey: key,
	}
	err = CreateCACertificate(cfg)
