...
import (
	...
	"github.com/Mydawn2/CLkey-common/crypto/asym/ecdsa"
	"github.com/Mydawn2/CLkey-common/crypto/asym/rsa"
...
func GenerateKeyPairBytes(keyType crypto.KeyType, uid []byte) (sk, pk []byte, err error) {
	var priv crypto.PrivateKey
	if priv, err = GenerateKeyPair(keyType, uid); err != nil {
		return
	}
...
func GenerateKeyPairPEM(keyType crypto.KeyType, uid []byte) (sk string, pk string, err error) {
	var priv crypto.PrivateKey
	if priv, err = GenerateKeyPair(keyType, uid); err != nil {
		return "", "", err
	}
...
func WriteFile(keyType crypto.KeyType, uid []byte, filePath string) error {
	sk, pk, err := GenerateKeyPairPEM(keyType, uid)
	if err != nil {
		return err
	}
