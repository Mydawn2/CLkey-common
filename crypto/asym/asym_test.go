...
func TestGenerateKeyPairPEM(t *testing.T) {
	...
	sk, pk, err = GenerateKeyPairPEM(crypto.RSA2048, []byte("mydawn"))
	require.Nil(t, err)
	fmt.Println("sk: ", sk)
	fmt.Println("pk: ", pk)
}
...
func testSignAndVerify(t *testing.T, opt crypto.KeyType) {
	...
	// 方式1：
	sk, pk, err := GenerateKeyPairPEM(opt, []byte("mydawn"))
	require.Nil(t, err)
	...
	// 方式2：
	sk2, pk2, err := GenerateKeyPairBytes(opt, []byte("mydawn"))
	require.Nil(t, err)
	...
	// 方式3：
	sk3, err := GenerateKeyPair(opt, []byte("mydawn"))
	require.Nil(t, err)
...
func testSignAndVerifyWithOpts(t *testing.T, opt crypto.KeyType) {
	...
	// 方式1：
	skPEM, pkPEM, err := GenerateKeyPairPEM(opt, []byte("mydawn"))
	require.Nil(t, err)
