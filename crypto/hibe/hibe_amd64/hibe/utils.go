...
import (
	...
	"github.com/Mydawn2/CLkey-common/crypto/hibe/hibe_amd64/hibe/bn256"
)
...
func (params *Params) Unmarshal(marshalled []byte) (*Params, bool) {
	...
	if _, ok := params.G.Unmarshal(geIndex(marshalled, 0, 2)); ok != nil {
		return nil, false
	}

	params.G1 = new(bn256.G2)
	if _, ok := params.G1.Unmarshal(geIndex(marshalled, 2, 2)); ok != nil {
		return nil, false
	}

	params.G2 = new(bn256.G1)
	if _, ok := params.G2.Unmarshal(geIndex(marshalled, 4, 1)); ok != nil {
		return nil, false
	}

	params.G3 = new(bn256.G1)
	if _, ok := params.G3.Unmarshal(geIndex(marshalled, 5, 1)); ok != nil {
		return nil, false
	}
	...
	for i := range params.H {
		hi := new(bn256.G1)
		params.H[i] = hi
		if _, ok := hi.Unmarshal(geIndex(marshalled, 6+i, 1)); ok != nil {
			return params, false
		}
	}
...
func (key *PrivateKey) Unmarshal(marshalled []byte) (*PrivateKey, bool) {
	...
	key.A0 = new(bn256.G1)
	if _, ok := key.A0.Unmarshal(geIndex(marshalled, 0, 1)); ok != nil {
		return nil, false
	}

	key.A1 = new(bn256.G2)
	if _, ok := key.A1.Unmarshal(geIndex(marshalled, 1, 2)); ok != nil {
		return nil, false
	}
	...
	for i := range key.B {
		bi := new(bn256.G1)
		key.B[i] = bi
		if _, ok := bi.Unmarshal(geIndex(marshalled, 3+i, 1)); ok != nil {
			return key, false
		}
	}
...
func (ciphertext *Ciphertext) Unmarshal(marshalled []byte) (*Ciphertext, bool) {
	...
	if _, ok := ciphertext.A.Unmarshal(geIndex(marshalled, 0, 6)); ok != nil {
		return nil, false
	}
	ciphertext.B = new(bn256.G2)
	if _, ok := ciphertext.B.Unmarshal(geIndex(marshalled, 6, 2)); ok != nil {
		return nil, false
	}
	ciphertext.C = new(bn256.G1)
	if _, ok := ciphertext.C.Unmarshal(geIndex(marshalled, 8, 1)); ok != nil {
		return nil, false
	}
