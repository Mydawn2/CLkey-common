...
import (
	...
	"chainmaker.org/chainmaker/common/v2/crypto/hibe/hibe_noamd64/hibe"
...
func TestV2(t *testing.T) {
	...
	var (
		paramsFilePath     = "./hibe-test-data/wx/wx.params"
		masterKeyFilePath  = "./hibe-test-data/wx/wx.masterKey"
		privateKeyFilePath = "./hibe-test-data/wx/privateKeys/topL.privateKey"
		topLId             = "topL"
		//id2 = ""
		//id3 = ""
	)
	...
	masterKeyBytes, err := ioutil.ReadFile(masterKeyFilePath)
	require.Nil(t, err)
	master := new(G1)
	_, err = master.Unmarshal(masterKeyBytes)
	require.Nil(t, err)
	...
	private, ok = private.Unmarshal(privateKeyBytes)
	require.True(t, ok)
	fmt.Printf("privateKey: %+v\n", private)

	ct := map[string]string{
		"CT":                  "MDoEDPQ+F0g4TR6WzyfbigQYIzUT8sErQXyE8MfPID9bD8rsBHqH+KeiBBDVRGwQqoxmXSMVB9GCuIBN",
		"topL/secondL/thirdL": "",
	}

	hibeMsg, err := DecryptHibeMsg(topLId, params, private, ct, crypto.AES)
	require.Nil(t, err)
	fmt.Printf("hibeMsg: %s\n", hibeMsg)
}
...
func testEncrypt2HibeMsgMap(plaintext []byte, symKeyType crypto.KeyType) (map[string]string, error) {
	receiverIds := make([]string, 4)
	receiverIds[0] = secondLId
	receiverIds[1] = topLId
	receiverIds[2] = thirdLId
	receiverIds[3] = otherLId
	...
	return EncryptHibeMsg(plaintext, receiverIds, paramsList, symKeyType)
...
func testEncrypt2HibeMsgMapWithUnsupportData(t *testing.T, plaintext []byte) {
	tests := []struct {
		...
		{ //
			...
			wantError:  "invalid parameters, plaintext is nil",
		},
		{ //
			plaintext:  plaintext,
			receiveIds: []string{},
			paramsList: make([]*hibe.Params, 0),
			symKeyType: crypto.ECC_Ed25519,
			wantError:  "invalid parameters, receiverIds is nil",
		},
		{ //
			plaintext:  plaintext,
			...
			paramsList: make([]*hibe.Params, 0),
			symKeyType: crypto.ECC_Ed25519,
			wantError:  fmt.Sprintf("invalid parameters, id: %s, format error, only like : \"A/B/C\" can be used", unsupportIdFmt),
		},
		{ //
			plaintext:  plaintext,
			...
			paramsList: make([]*hibe.Params, 0),
			symKeyType: crypto.ECC_Ed25519,
			wantError:  "invalid parameters, paramsList is nil",
		},
		{ //
			plaintext:  plaintext,
			receiveIds: []string{secondLId},
			...
			symKeyType: crypto.ECC_Ed25519,
			wantError:  "invalid parameters, receiverIds and paramsList do not match, place check them",
		},
		{ // UnsupportSymKey
			plaintext:  plaintext,
			receiveIds: []string{secondLId},
			...
			symKeyType: crypto.ECC_Ed25519,
			wantError:  fmt.Sprintf("invalid parameters, unsupported symmetric encryption algorithm type : %d", crypto.ECC_Ed25519),
		},
		{ //
			plaintext:  plaintext,
			...
			wantError:  fmt.Sprintf("ID [%s] match, but Params are different, please check it", topLId),
		},
		{ //
			plaintext:  plaintext,
			...
			paramsList: []*hibe.Params{params, params2},
			symKeyType: crypto.AES,
			wantError:  fmt.Sprintf("ID [%s] is matched, but Params are different, please check it", secondLId),
		},
	}
