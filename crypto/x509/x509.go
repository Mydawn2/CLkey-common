...
import (
	...
	bccrypto "github.com/Mydawn2/CLkey-common/crypto"
	"github.com/Mydawn2/CLkey-common/crypto/asym"
	bcecdsa "github.com/Mydawn2/CLkey-common/crypto/asym/ecdsa"
	bcrsa "github.com/Mydawn2/CLkey-common/crypto/asym/rsa"
...
var signatureAlgorithmDetails = []struct {
	...
	{SHA256WithSM2, "SM2-SHA256", oidSignatureSHA256WithSM2, ECDSA, crypto.SHA256},
	{SM3WithSM2, "SM2-SM3", oidSignatureSM3WithSM2, ECDSA, crypto.Hash(bccrypto.HASH_TYPE_SM3)},
}
...
// CheckSignature verifies that signature is a valid signature over signed from
// a libp2pcrypto.PublicKey.
func checkSignature(algo SignatureAlgorithm, signed, signature []byte, publicKey bccrypto.PublicKey) (err error) {
	hashAlgo, err := GetHashFromSignatureAlgorithm(algo)
	if err != nil {
		return err
	}

	ok, err := publicKey.VerifyWithOpts(signed, signature, &bccrypto.SignOpts{
		Hash: hashAlgo,
...
func forEachSAN(extension []byte, callback func(int, []byte) error) error {
	// RFC 5280, 4.2.1.6
...
func parsePublicKey(algo PublicKeyAlgorithm, keyData *publicKeyInfo) (bccrypto.PublicKey, error) {
	...
	case RSA:
		...
		return &bcrsa.PublicKey{K: pub}, nil
	case ECDSA:
		...
		if namedCurveOID.Equal(oidNamedCurveSm2) {
			pk, err := asym.ParseSM2PublicKey(asn1Data)
			if err != nil {
				return nil, fmt.Errorf("fail to unmarshal sm2 public key")
			}
			return &bcecdsa.PublicKey{K: pk}, nil
		} else {
			...
			return &bcecdsa.PublicKey{K: pub}, nil
		}
	case SM2:
		fmt.Printf("SM2\n")
		paramsData := keyData.Algorithm.Parameters.FullBytes
		...
		pk, err := sm2.ParseSm2PublicKey(asn1Data)
		if err != nil {
			pk, err = asym.ParseSM2PublicKey(asn1Data)
			if err != nil {
				return nil, errors.New("fail to unmarshal sm2 public key")
			}
		}
		return &bcecdsa.PublicKey{K: pk}, nil
...
func buildExtensions(template *x509.Certificate) (ret []pkix.Extension, err error) {
			...
			dp := distributionPoint{
				DistributionPoint: distributionPointName{
					FullName: []asn1.RawValue{
						{Tag: 0, Class: 2, IsCompound: true, Bytes: rawFullName},
					},
				},
			}
...
func GetHashFromSignatureAlgorithm(algo SignatureAlgorithm) (bccrypto.HashType, error) {
	for _, details := range signatureAlgorithmDetails {
		if details.algo == algo {
			return bccrypto.HashType(details.hash), nil
		}
	}
	return 0, fmt.Errorf("certificate: invalid algorithm suite")
}

func ChainMakerCertToX509Cert(cert *Certificate) (*x509.Certificate, error) {
	newCert := &x509.Certificate{
		...
		PublicKey:                   cert.PublicKey.ToStandardKey(),
...
func X509CertToChainMakerCert(cert *x509.Certificate) (*Certificate, error) {
...
func GetExtByOid(oid asn1.ObjectIdentifier, extensions []pkix.Extension) ([]byte, error) {
	...
	return nil, fmt.Errorf("extension with given oid does not exist")
}

func GetAKIFromExtensions(extensions []pkix.Extension) ([]byte, bool, error) {
	akiASN1, err := GetExtByOid(oidExtensionAuthorityKeyId, extensions)
	if err != nil {
		return nil, false, err
	}
	var aki authKeyId
	_, err = asn1.Unmarshal(akiASN1, &aki)
	if err != nil {
		return akiASN1, false, nil
	}
	return aki.Id, true, nil
}

func GetNodeIdFromCertificate(oid asn1.ObjectIdentifier, cert x509.Certificate) ([]byte, error) {
	id, err := GetExtByOid(oid, cert.Extensions)
	if err != nil || id == nil {
		id = []byte(cert.Subject.CommonName)
		if id == nil || len(id) <= 0 {
			return nil, fmt.Errorf("given certificate does not contain node ID")
		}
	}
	return id, nil
}

func GetNodeIdFromSm2Certificate(oid asn1.ObjectIdentifier, cert sm2.Certificate) ([]byte, error) {
	id, err := GetExtByOid(oid, cert.Extensions)
	if err != nil || id == nil {
		id = []byte(cert.Subject.CommonName)
		if id == nil || len(id) <= 0 {
...
func signingParamsForPublicKey(pub interface{}, requestedSigAlgo SignatureAlgorithm) (hashFunc crypto.Hash, sigAlgo pkix.AlgorithmIdentifier, err error) {
	...
	case *sm2.PublicKey:
		...
		hashFunc = crypto.Hash(bccrypto.HASH_TYPE_SM3)
...
func marshalPublicKey(pub interface{}) (publicKeyBytes []byte, publicKeyAlgorithm pkix.AlgorithmIdentifier, err error) {
	...
	case *ecdsa.PublicKey:
		...
		if !ok {
			return nil, pkix.AlgorithmIdentifier{}, errors.New("x509: unsupported ecdsa.PublicKey elliptic curve")
		}
	...
	case *sm2.PublicKey:
		...
		oid, ok := oidFromNamedCurve(pub.Curve)
		if !ok {
			return nil, pkix.AlgorithmIdentifier{}, errors.New("x509: unsupported marshal sm2 public key")
		}
		...
		paramBytes, err = asn1.Marshal(oid)
		if err != nil {
			return nil, pkix.AlgorithmIdentifier{}, errors.New("asn1 marshal sm2 oid failed")
		}
		publicKeyAlgorithm.Parameters.FullBytes = paramBytes
	default:
...
func CreateCertificate(rand io.Reader, template, parent *x509.Certificate, pub, priv interface{}) (cert []byte, err error) {
	...
	hashFunc, signatureAlgorithm, err := signingParamsForPublicKey(key.Public(), SignatureAlgorithm(template.SignatureAlgorithm))
	if err != nil {
		return nil, err
	}
	...
	var digest []byte
	if hashFunc == crypto.Hash(bccrypto.HASH_TYPE_SM3) {
		//当SM3时取ZA方式签名,所以不在这里做hash
...
func CreateCertificateRequest(rand io.Reader, template *CertificateRequest, priv interface{}) (csr []byte, err error) {
	...
	hashFunc, sigAlgo, err = signingParamsForPublicKey(key.Public(), SignatureAlgorithm(template.SignatureAlgorithm))
	if err != nil {
		return nil, err
	}
...
func parseCertificateRequest(in *certificateRequest) (*CertificateRequest, error) {
	...
	var err error
	out.PublicKey, err = parsePublicKey(PublicKeyAlgorithm(out.PublicKeyAlgorithm), &in.TBSCSR.PublicKey)
	if err != nil {
		return nil, err
	}
...
func ChainMakerCertCsrToX509CertCsr(cert *CertificateRequest) (*x509.CertificateRequest, error) {
	var pk crypto.PublicKey
	if cert.PublicKey == nil {
		pk = nil
	} else {
		pk = cert.PublicKey.ToStandardKey()
	}
...
func X509CertCsrToChainMakerCertCsr(cert *x509.CertificateRequest) (*CertificateRequest, error) {
	var pk bccrypto.PublicKey
	if cert.PublicKey != nil {
		der, err := MarshalPKIXPublicKey(cert.PublicKey)
		if err != nil {
			return nil, fmt.Errorf("fail to parse re-encode (marshal) public key in certificate: %v", err)
		}
		pk, err = asym.PublicKeyFromDER(der)
...
func BuildCertificateChain(certs []*Certificate) []*Certificate {
	...
	for _, cert := range certs {
		if cert.IsCA {
			...
			nonCaCerts = append(nonCaCerts, cert)
			intermediatePool.AddCert(cert)
		}
	}
	...
	if len(nonCaCerts) <= 0 {
		for i, certCandidate := range caCerts {
			intermediateCAPool := NewCertPool()
			for j, cert := range caCerts {
				if i != j {
					intermediateCAPool.AddCert(cert)
				}
			}
			candidateChains, err := certCandidate.Verify(VerifyOptions{
				DNSName:                   "",
		...
		for _, cert := range nonCaCerts {
			candidateChains, err := cert.Verify(VerifyOptions{
				DNSName:                   "",
			...
			candidateTmp := candidateChains[0]
			for _, candidate := range candidateChains {
				if len(candidate) > len(candidateTmp) {
					candidateTmp = candidate
				}
			}
			if len(candidateTmp) > len(chain) {
				chain = candidateTmp
			}
		}
	}
...
//CreateCRL .ceate crl
func CreateCRL(rand io.Reader, cert *x509.Certificate, priv interface{}, revokedCerts []pkix.RevokedCertificate, now, expiry time.Time) (crlBytes []byte, err error) {
	key, ok := priv.(crypto.Signer)
	if !ok {
		return nil, errors.New("x509: certificate private key does not implement crypto.Signer")
	}

	hashFunc, signatureAlgorithm, err := signingParamsForPublicKey(key.Public(), SignatureAlgorithm(cert.SignatureAlgorithm))
	if err != nil {
		return nil, err
	}
	...
	if len(cert.SubjectKeyId) > 0 {
		...
		aki.Value, err = asn1.Marshal(authKeyId{Id: cert.SubjectKeyId})
		if err != nil {
			return
		}
