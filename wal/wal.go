...
func (l *Log) load() error {
	...
	if err := l.loadSegmentEntriesForRestarting(lseg); err != nil {
		return err
	}
	// Customize part end
...
func appendJSONEntry(dst []byte, index uint64, data []byte) (out []byte,
	epos bpos) {
	// {"index":number,"checksum":checksum,"data":string}
	mark := len(dst)
	dst = append(dst, `{"index":"`...)
	dst = strconv.AppendUint(dst, index, 10)
	// Customize part start
	dst = append(dst, `","checksum":"`...)
	dst = strconv.AppendUint(dst, uint64(NewCRC(data).Value()), 10) // checksum is calculated by original data
	// Customize part end
	dst = append(dst, `","data":`...)
...
// loadSegmentEntries loads ebuf and epos in the segment for normal
...
func (l *Log) loadSegmentEntriesForRestarting(s *segment) error {
	data, err := ioutil.ReadFile(s.path)
	if err != nil {
		return err
	}
...
func readJSON(edata []byte) ([]byte, error) {
	var data []byte
	// Customize part start
	cs := gjson.Get(*(*string)(unsafe.Pointer(&edata)), "checksum").String()
	// Customize part end
	...
	if len(s) > 0 && s[0] == '$' {
		...
		// Customize part start
		if cs != strconv.FormatUint(uint64(NewCRC(data).Value()), 10) {
			return nil, ErrCorrupt
		}
		// Customize part end
	} else if len(s) > 0 && s[0] == '+' {
