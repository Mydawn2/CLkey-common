...
func TestOutliers(t *testing.T) {
	...
	t.Run("corrupted-tail-binary", func(t *testing.T) {
		...
		ioutil.WriteFile(
			"testlog/corrupt-tail/00000000000000000001",
			testGenBinaryData([]byte("string"), true, false, false), 0666) // modify checksum
		if l, err := Open("testlog/corrupt-tail", opts); err != nil {
			l.Close()
			t.Fatalf("expected %v, got %v", "not err", err)
		}
		...
		ioutil.WriteFile(
			"testlog/corrupt-tail/00000000000000000001",
			testGenBinaryData([]byte("string"), false, true, false), 0666) // modify data_size
		if l, err := Open("testlog/corrupt-tail", opts); err != nil {
			l.Close()
			t.Fatalf("expected %v, got %v", "not err", err)
		}
		...
		ioutil.WriteFile(
			"testlog/corrupt-tail/00000000000000000001",
			testGenBinaryData([]byte("string"), false, false, true), 0666) // modify data
		if l, err := Open("testlog/corrupt-tail", opts); err != nil {
			l.Close()
			t.Fatalf("expected %v, got %v", "not err", err)
		}
	...
	t.Run("uncorrupted-tail-binary", func(t *testing.T) {
		...
		ioutil.WriteFile(
			"testlog/uncorrupted-tail/00000000000000000001",
			testGenBinaryData([]byte("string"), false, false, false), 0666)
		l, err := Open("testlog/uncorrupted-tail", opts)
		if err != nil {
			l.Close()
			t.Fatalf("expected %v, got %v", "not err", err)
